...
if __name__ == "__main__":
    ...
    df =  pd.read_csv(os.path.join(os.path.dirname(filepath),'actr_temp.csv'))
    df['actr_data_processed'] = df.progress_apply(lambda x: process_actr_data(x['actr_data']), axis=1)
    df.info()
    df.to_csv(os.path.join(os.path.dirname(filepath),'actr_sim_%s.csv' %(target)), index=False)
    # delete temporary file
    os.remove(os.path.join(os.path.dirname(filepath),'actr_temp.csv'))